 ...
 * <li>selectionMode : sap.ui.table.SelectionMode (default: 'Single') The selection mode allowed for the table control.</li>
...
sap.ui.core.Control.extend("controls.w.DynamicTable", {
	...
	updateTableColumns: function() {
		...
		this.getTable().bindAggregation("columns", "/columns", function(index, context) {
			...
			// If checkbox needed, 
			if (context.getObject().columnId === "selectRowCheckBox") {
				var loCheckBox = new sap.m.CheckBox({
					selected: context.getObject().selected,
					select: function(ioEvent) {
						ioEvent.getSource().getBindingContext().getObject().selected = true;
						_self.fireOnSelectTableRowCheckBox(ioEvent);
					}
				});
