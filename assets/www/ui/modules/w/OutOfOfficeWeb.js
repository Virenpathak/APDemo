//Declare
jQuery.sap.declare("modules.w.OutOfOfficeWeb");
//Require
jQuery.sap.require("modules.SettingsIconAction");

//CONSTRUCTOR
modules.w.OutOfOfficeWeb = function(options) {
...
modules.w.OutOfOfficeWeb.prototype = {
		defaultSettings: {
			page: ""
		},
...
//				iCheck = 0;
				...
				var loSDatePicker = ioEvent.getSource().getContent()[0].getItems()[0].getItems()[0].getItems()[1];
				var loEDatePicker = ioEvent.getSource().getContent()[0].getItems()[0].getItems()[1].getItems()[1];
...
//				for (var i = 1; i < this.getPanelContainer().getItems()[0].getItems().length; i++){

//				var laVContent = this.getPanelContainer().getItems()[0].getItems()[i].getContent()[0].getItems();
//				var loSdate = laVContent[0].getItems()[0].getItems()[1].setValue(oDate);
////				var loEdate = laVContent[0].getItems()[1].getItems()[1];
				...
				for (var i = 0; i < loData.d.results.length; i++) {
					...
					if (loDocType === loGetDocType) {
						...
						if (ioEvent.getSource().getContent().length != 0) {
							...
							var loInput = ioEvent.getSource().getContent()[0].getItems()[k].getItems()[3];
...
//					}
					var loSelectSub1 = loBlock[i].getContent()[0].getItems()[3].getItems()[1];
					var loSelectSub2 = loBlock[i].getContent()[0].getItems()[4].getItems()[1];
					if (loSelectSub1.getEnabled() && loSelectSub2.getEnabled()){
						if (loSelectSub1.getSelectedItem().getText() === loSelectSub2.getSelectedItem().getText()) {
							sap.m.MessageToast.show("Please Select Different Substitute");//goBundle.getText("VALIDATION") :TODO
					...
					if (this.getPanelContainer().getItems()[0].getItems()[i].getContent().length !== 0) {
						for (var j = 3; j < this.getPanelContainer().getItems()[0].getItems()[i].getContent()[0].getItems().length; j++) {
							...
							if (loCheckBox.getSelected() === true) {
								...
								var loReason = laVContent[2].getValue();
								var loSelect = loItems.getItems()[1];
								var loTitle = "";
								if (loSelect.getEnabled() && loLabel != "Select Substitute  "){			
									...
									var loID = loItems.getItems()[3].getValue();
									var loFname = " ";
									var loLname = " ";
									var loUserfName = "";
									var loUserlName = "";
